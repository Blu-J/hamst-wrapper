...
clean:
	rm -rf hamster/.quasar
	rm -rf hamster/node_modules
	rm -rf hamster/dist
	rm -rf docker-images
	rm -f image.tar
	rm -f $(PKG_ID).s9pk
...
docker-images/aarch64.tar: Dockerfile docker_entrypoint.sh hamstr/dist
ifeq ($(ARCH),x86_64)
else
...
docker-images/x86_64.tar: Dockerfile docker_entrypoint.sh hamstr/dist
ifeq ($(ARCH),aarch64)
else
	mkdir -p docker-images
